...
- [ ] ~~An eval covers it offline: the stub's `slow-silent` mode plus a grader asserting the kill command follows `TaskStop` in the trace, since the process check itself cannot run in the eval sandbox.~~ Superseded (reliability slice 03): the spec fixes the opposite order — the stop script runs *before* `TaskStop` — and the offline graders are `used-run-script`, `used-stop-script`, `stopped-at-stop` (the script's report line, ending `process tree ended`, written by the assistant before the `TaskStop` call) and `stopped-line` (restated in the final reply).

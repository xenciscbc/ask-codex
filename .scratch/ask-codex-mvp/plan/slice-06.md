...
Ticket: `.scratch/ask-codex-mvp/issues/06-follow-up-consultation.md` (spec stories **63–65**; the ticket's "52–54" reference is a numbering slip; ADR-0002). Envelope, F-map, lessons, stops: `PLAN.md`. Blocked by 02 (resolved). Serial order: after 05. **Entry gate (changed 2026-09-16 by user decision):** ticket 05 is paused (proactive proposal not yet reliable; safety held); ticket 06 starts on top of the committed paused ticket-05 state. The Proactive-section sentence this slice edits is part of that state; `review-loop-blockers` stays a regression case. No dollar cap (user decision); 0 live Codex calls.

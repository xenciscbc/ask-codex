...
- **Checks around every run.** Before and after each headless run, each direct `codex exec`, and each interactive session: compare presence and hash with the baseline. On any change: diff it against the backup first, then **stop before the next Codex call** and restore the backup — *except* for a change confined to the Codex app's own runtime bookkeeping in `config.toml` (the `cua_node` runtime directory hash, `BROWSER_USE_CODEX_APP_VERSION`, the `SKY_CUA_NATIVE_PIPE_DIRECTORY` GUID, `CODEX_CLI_PATH` and the matching `node_repl`/`cua_repl` paths), which the app rewrites when it updates; restoring those would point the servers at a deleted runtime. Such a change is re-baselined (fresh backup and hash) and recorded, and the run continues. Any change to `model`, `model_reasoning_effort`, `approval_policy`, `sandbox_mode`, or to the membership or `enabled` state of `mcp_servers` is a stop, never a re-baseline. **Precedent:** the app rewrote exactly those runtime keys at 08:09 on 2026-09-16, between the first baseline and the first scenario; the user chose to re-baseline (recorded in ticket 09).

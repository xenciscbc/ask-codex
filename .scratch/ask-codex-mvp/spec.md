...
- Runner: runs where `claude plugin eval` can sandbox Bash — on this machine inside WSL (Ubuntu, Claude Code 2.1.272), because the Windows harness refuses shell grants without a Windows sandbox. It copies the stub `codex` to locations the agent's Bash sandbox can see (the sandbox hides `/tmp` and `/mnt/*`), removes every `PATH` entry containing a real `codex`, prepends the stub locations, fails fast unless `codex` resolves to a stub, then runs `claude plugin eval` with `--scaffold`, `--no-publish`, `--ablation none` during development, `--trust-plugin` when the Claude Code version supports it, and a limited tool grant. An optional temp-root override keeps the eval sandbox off drives that cannot host it.
...
- Eval facts confirmed while building the ticket-01 harness: the Windows `claude plugin eval` refuses any Bash grant ("sandbox required but unavailable"), so Tier 1 runs in WSL; the eval harness cannot create its sandbox on a RAM-disk temp drive; the agent's Bash sandbox hides `/tmp` and `/mnt/*`; `AskUserQuestion` does not exist inside eval children — the skill therefore asks in plain text and stops when `AskUserQuestion` is unavailable, consent cases assert "no `codex` call + consent requested in text", post-consent behaviour is seeded via `context.history_file`, and the real three-option flow is covered by live acceptance. Still unconfirmed: whether a scaffold can seed a model cache into the throwaway Codex home (ticket 03).

...
- [ ] ticket 12 resolved（Comments 引用 03 的 eval 結果與本 ticket 的 live 證據）；ticket 15、16 的 Comments 已補上 live 證據指標。
- [ ] `config.toml` hash 前後一致（Codex app 自行改寫的欄位除外，逐一列出——每個 scenario 開始前先備份整個檔案的位元組，且在 Codex app 啟動完成後才取 hash；見 evidence/01 的教訓）；probe workspace 已刪除。

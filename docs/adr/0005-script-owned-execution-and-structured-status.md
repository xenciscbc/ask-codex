...
Model and effort selection later moved into the script as well, with its behavior unchanged. Claude reads the model tokens off the request, because telling a model token from the first words of a question is judgment. A `resolve` operation takes those tokens, reads the model cache and configuration, validates and matches them, applies the effort rules, default order and parallel-pair limits, and reports whether a named choice differs from the session and configuration baseline. Claude keeps the user interaction: asking on an ambiguous alias, the scope question, and disclosing notes. The two per-model default efforts (`sol` → high, `astra` → medium) are data in the script, the only fixed alias entries.

...
Model and effort selection later moved into the script as well, with its behavior unchanged. A `resolve` operation reads the model cache and configuration, parses the request's model tokens and parallel pair, applies the effort rules and default order, and reports whether a named choice differs from the session and configuration baseline. Claude keeps the user interaction: asking on an ambiguous alias, the scope question, and disclosing notes. The two per-model default efforts (`sol` → high, `astra` → medium) are data in the script, the only fixed alias entries.

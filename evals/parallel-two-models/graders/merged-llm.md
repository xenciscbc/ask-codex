...
Two models answered the same question. Both said fetchUser catches TimeoutError and returns an empty object (consensus). gpt-6-astra alone suggested a circuit breaker; gpt-6-sol alone suggested an AbortController signal (solo claims). They disagree on whether raising the timeout would fix the problem (divergence).

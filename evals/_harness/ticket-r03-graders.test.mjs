...
const SUFFIX = "; done — gpt-6-sol; still running — gpt-6-astra";
...
for (const [label, text, ok] of [
  ["line + suffix", LINE + SUFFIX, true], ["bold + backticked slugs + hyphen", BOLD + "; done - `gpt-6-sol`; still running - `gpt-6-astra`", true],
  ["no suffix", LINE, false], ["swapped models", LINE + "; done — gpt-6-astra; still running — gpt-6-sol", false],
...
const STOP_CMD = { command: "bash '/p/skills/ask/scripts/stop.sh' '/t/ask-codex/run.a' --interval 1 --interval-source override --recommended stop --done 'gpt-6-sol' --still-running 'gpt-6-astra'" };
...
const NOTICE_THEN_STOP = trace(A("Codex still running — 1:02 elapsed; done — gpt-6-sol; still running — gpt-6-astra; waiting another 1 minutes."), TOOL("Bash", STOP_CMD), RESULT(LINE + SUFFIX), TASKSTOP, A(LINE + SUFFIX));

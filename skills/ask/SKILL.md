...
2. At the start of the request, `model <x>` or `use <x>` names a model. Otherwise the first token names one if it contains `:` or its head matches a listed slug or contiguous hyphen/dot-separated slug parts. A next word matching slug parts joins it, as in `5.6 sol`. Other text is the question. Validate model tokens against `^[A-Za-z0-9._:-]+( [A-Za-z0-9._-]+)?$`; validate effort tokens as lowercase letters. Invalid tokens stop before Codex calls.

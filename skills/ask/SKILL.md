...
The `run.sh` prefix ships with this skill: it starts Codex as its own process tree and writes `<tmp>/pid` so the stop script can end it, and `<tmp>/exit-code` when the command ends so the wait script can see it; everything after `--` is the `codex exec` command with its redirections, unchanged. Omit `-m <model>` only when step 6 found no model. Never add other flags or `-c` keys (no `--dangerously-*`, `--full-auto`, `--yolo`, `--profile`, `--add-dir`, `--ignore-user-config`, other sandbox modes, `resume`, or `fork`).
...
**Check when T has passed.** Judge liveness only from the tracked task's state — without `TaskOutput`, from the wait script's line — and the last event in `<tmp>/events.jsonl` — never from CPU use or process listings:

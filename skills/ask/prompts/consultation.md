...
4. **Answer format.** Reply only with JSON matching the provided schema:
   ...
   - `claims`: each a single assertion with a stable `id` (C1, C2, …), `kind` = `fact` (directly observed) or `inference` (reasoned), `confidence` (`high`/`medium`/`low`), `evidence` (`file:line` references or short command/output excerpts; empty only if none exist), and `followup_status` = `null` unless the Consultation type section above tells you to report follow-up statuses.
